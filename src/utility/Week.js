...
export default class Week {
  constructor(year, month, ordinal) {
    ...
    const d = new Day(fd.year, fd.month, fd.date + 7 * (ordinal - 1));
    if (ordinal < 1 || (ordinal > 1 && d.month !== month)) {
      throw new Error(`引数の値が不正です。: ordinal = ${ordinal}`);
    }
    this._w = times(7).map((_, i) => new Day(d.year, d.month, d.date + i));
    this._y = year;
    this._m = month;
    this._o = ordinal;
  }
  ...
  isEndOfMonth() {
    const d = this._w[0];
    return this._m !== new Day(d.year, d.month, d.date + 7).month;
  }
}
